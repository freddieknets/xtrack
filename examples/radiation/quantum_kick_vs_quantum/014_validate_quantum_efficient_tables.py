...
# Production table generation targets direct fixed-count tables through N=32.
# That is too expensive for a routine validation example because each direct
# count requires an offline convolution. This example validates the same direct
# table machinery on a small direct range, then uses the power-of-two fallback
# for larger counts. Full N=32 generation should be run deliberately when
# producing the C header or doing final certification.
VALIDATION_DIRECT_TABLE_MAX = 4
...
def build_log_inverse_cdf_tables(max_power = 256,
                                 direct_table_max=VALIDATION_DIRECT_TABLE_MAX):
    ...
    direct_table_max : int
        Largest direct fixed-count table to generate for this validation run.
        The production target is 32, but this example defaults to a smaller
        value to keep the validation practical.
    ...
    Notes
    -----
    This function calls the production table generator with a reduced direct
    table range. It checks the same deterministic direct-table path without
    requiring all 32 direct fixed-count convolutions on every validation run.
    The returned values are converted to logarithms because the runtime C
    header stores ``log(X_N)`` and interpolates in log-space.
    """
    u_grid, quantile_tables = table_generator.build_total_energy_quantile_tables(
        u_grid=table_generator.make_segmented_probability_grid(),
...
def decompose_photon_count(n_photons, tables):
    ...
    Notes
    -----
    This mirrors the table32 runtime structure for whatever direct-table range
    is available in this validation run. Counts within the direct range use one
    direct table. Larger counts are represented by available high-count chunks
    plus a final direct remainder.
    """
    if n_photons == 0:
        return []
    ...
    direct_table_max = 0
    while direct_table_max + 1 in tables:
        direct_table_max += 1

    chunks = []
    high_chunks = sorted(
        nn for nn in tables
        if nn > direct_table_max and nn & (nn - 1) == 0)

    if n_left <= direct_table_max:
        return [n_left]

    while n_left > direct_table_max:
        candidates = [nn for nn in high_chunks if nn <= n_left]
        if candidates:
            chunk = candidates[-1]
        else:
            chunk = direct_table_max
        chunks.append(chunk)
        n_left -= chunk
...
def sample_one_table_chunk(rng, u_grid, log_table):
    ...
    """
    u = rng.random()
    return np.exp(table_generator.interpolate_segmented_log_quantile(
        np.array([u]), u_grid, log_table)[0])
...
print(f"VALIDATION_DIRECT_TABLE_MAX = {VALIDATION_DIRECT_TABLE_MAX}")
